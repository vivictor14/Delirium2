...
move(X, Y, 0, Laby, Action) :- meilleureOption([X, Y], Laby, 2, Coord), write(X), write(' '), write(Y), nl, write('Je me dirige vers un diamant'), nl, seDirigerVers([X, Y], Coord, Laby, _, Action), !.
